...
class ChannelPool(nn.Module):
    def forward(self, x):
        # torch.max（x，1）按照行取最大值(取一列中最大值)，最后得到最大值【0】，unsqueeze(1),第二维查一个1维，变成（x，1，y），把原矩阵按照变成x个通道
        # 按照第二维拼接起来
...
class BiFusion_block(nn.Module):
    def __init__(self, ch_1, ch_2, r_2, ch_int, ch_out, drop_rate=0.):
        ...
        # channel attention for F_g, use SE Block   se模块的通道注意力
        self.fc1 = nn.Conv2d(ch_2, ch_2 // r_2, kernel_size=1)  # 通道变了,也卷积了
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Conv2d(ch_2 // r_2, ch_2, kernel_size=1)
        self.sigmoid = nn.Sigmoid()  # 归一（0，1）

        # spatial attention for F_l  空间注意力机制
        self.compress = ChannelPool()
        # conv是自己写的一个卷积层，包含有卷积、归一、relu激活
        self.spatial = Conv(2, 1, 7, bn=True, relu=False, bias=False)

        # bi-linear modelling for both   双线性插值
    ...
    def forward(self, g, x):
        ...
        W_g = self.W_g(g)
        W_x = self.W_x(x)
        bp = self.W(W_g * W_x)  # cnn和trans的相乘
...
def window_partition(x, window_size):
    ...
    """
    B, H, W, C = x.shape
    x = x.view(B, H // window_size, window_size, W // window_size, window_size, C)   #6个通道
...
class Cross_Att(nn.Module):
    def __init__(self, dim_s, dim_l):
        ...
        self.spatial = Conv(2, 1, 7, bn=True, relu=False, bias=False)
        self.relu = nn.ReLU(inplace=True)
        self.fc1 = nn.Conv2d(dim_s, 16, kernel_size=1)  # 通道变了,也卷积了
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Conv2d(16, dim_s, kernel_size=1)
    def forward(self, e, r):
       b_e, c_e, h_e, w_e = e.shape    #(1,3,384,384)
...
class UNet1(nn.Module):
    def __init__(self, dim, n_class, in_ch=3):
        ...
        #                                 embed_dim=96)
        self.encoder3 = models.resnet34(pretrained=False)  # 将其中一个编码器换成预训练的Resnet-34网络
    ...
    def forward(self, x):
        out3 = self.encoder3.conv1(x)  # (2,64,32,32)
        out3 = self.encoder3.bn1(out3)  # (2,64,32,32)
        out3 = self.encoder3.relu(out3)  # (2,64,32,32)
        out3 = self.encoder3.maxpool(out3)  # [2, 64, 16, 16]
        # cnn
        out3 = self.encoder3.layer1(out3)  # [2, 64, 16, 16]
        e1 = self.encoder3.layer2(out3)  # [2, 128, 8, 8]
        e2 = self.encoder3.layer3(e1)  # [2, 256, 4, 4]
        e3 = self.encoder3.layer4(e2)  # 2,512,2,2
        ...
        e1 = self.change1(e1) # (1,128,96,96)

        e2 = self.change2(e2)#4, 256, 48, 48

        e3 = self.change3(e3)#4, 512, 24, 24

        e4 = self.change4(e4)#4, 1024, 12, 12
