...
class CrossEfficientViT(nn.Module):
    ...
    def forward(self, img):
        ...
        print(f"Time taken for feature extraction: {elapsed_time:.4f} seconds")
        print('1',sm_tokens.shape)
        print('2', lg_tokens.shape)
        sm_tokens, lg_tokens = self.multi_scale_encoder(sm_tokens, lg_tokens)

        print('3',sm_tokens.shape)
        print('4', lg_tokens.shape)

        sm_cls, lg_cls = map(lambda t: t[:, 0], (sm_tokens, lg_tokens))
        print('1',sm_cls.shape)
        print('2', lg_cls.shape)
        lg_cls1=self.fc1(lg_cls)
        com1=lg_cls1+sm_cls

        sm_logits = self.sm_mlp_head(sm_cls)
        lg_logits = self.lg_mlp_head(lg_cls)

        com=sm_logits+lg_logits
